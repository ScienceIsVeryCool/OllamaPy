...
class SkillValidator:
    ...
    def validate_skill_data(self, skill_data: Dict[str, Any]) -> ValidationResult:
        ...
        if "vibe_test_phrases" in skill_data:
            vibe_validation = self._validate_vibe_phrases(
                skill_data["vibe_test_phrases"]
            )
        ...
        if "function_code" in skill_data:
            code_validation = self._validate_function_code(
                skill_data["function_code"], skill_data.get("parameters", {})
            )
    ...
    def _validate_skill_name(self, name: str) -> ValidationResult:
        ...
        if not re.match(r"^[a-zA-Z_][a-zA-Z0-9_]*$", name):
            errors.append(
                "Skill name must be a valid Python identifier (no spaces or special characters except underscore)"
            )
        ...
        if name[0].isupper():
            warnings.append(
                "Skill names typically use camelCase or snake_case (starting with lowercase)"
            )
    ...
    def _validate_description(self, description: str) -> ValidationResult:
        ...
        if not description.strip():
            warnings.append(
                "Description is empty - consider adding a clear description of when to use this skill"
            )
            return ValidationResult(True, errors, warnings)

        if len(description) < 10:
            warnings.append(
                "Description is very short - consider providing more detail"
            )
        ...
        if not any(
            word in description.lower() for word in ["when", "use", "for", "to"]
        ):
            warnings.append(
                "Description should clearly indicate when this skill should be used"
            )
    ...
    def _validate_role(self, role: str) -> ValidationResult:
        ...
        if role not in self.valid_roles:
            errors.append(
                f"Invalid role '{role}'. Valid roles: {', '.join(self.valid_roles)}"
            )
    ...
    def _validate_parameters(self, parameters: Dict[str, Any]) -> ValidationResult:
        ...
        for param_name, param_info in parameters.items():
            ...
            if not re.match(r"^[a-zA-Z_][a-zA-Z0-9_]*$", param_name):
                errors.append(
                    f"Parameter name '{param_name}' must be a valid Python identifier"
                )
            ...
            if "type" not in param_info:
                ...
                errors.append(
                    f"Parameter '{param_name}' has invalid type '{param_info['type']}'. Valid types: {', '.join(self.valid_parameter_types)}"
                )
            ...
            if "required" in param_info and not isinstance(
                param_info["required"], bool
            ):
                errors.append(
    ...
    def _validate_vibe_phrases(self, vibe_phrases: List[str]) -> ValidationResult:
        ...
        if not vibe_phrases:
            warnings.append(
                "No vibe test phrases provided - the AI won't know when to use this skill"
            )
            return ValidationResult(True, errors, warnings)

        if len(vibe_phrases) < 2:
            warnings.append(
                "Consider adding more vibe test phrases for better AI recognition"
            )
    ...
    def _validate_function_code(
        ...
        if execute_func and parameters:
            param_validation = self._validate_execute_signature(
        ...
        if "log(" not in code:
            warnings.append(
                "Function should use log() to output results that the AI can see"
            )
    ...
    def _validate_execute_signature(
        self, func_node: ast.FunctionDef, parameters: Dict[str, Any]
    ) -> ValidationResult:
        ...
        for param_name in parameters.keys():
            if param_name not in func_args:
                if parameters[param_name].get("required", False):
                    errors.append(
                        f"Required parameter '{param_name}' not found in execute function signature"
                    )
                else:
                    warnings.append(
                        f"Optional parameter '{param_name}' not found in execute function signature"
                    )
        ...
        for arg_name in func_args:
            if arg_name not in param_names:
                warnings.append(
                    f"Function argument '{arg_name}' not declared in parameters"
                )
