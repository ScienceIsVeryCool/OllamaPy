...
class TestPackageStructure:
    ...
    def test_skill_editor_subpackage(self):
        ...
        skill_editor_root = Path(__file__).parent.parent / "src" / "ollamapy" / "skill_editor"
...
class TestProjectConfiguration:
    ...
    def test_entry_points_configuration(self):
        ...
        assert 'ollamapy' in config['project']['scripts'], "ollamapy script entry point missing"
        
        entry_point = config['project']['scripts']['ollamapy']
        assert entry_point == 'src.ollamapy.main:main', "Incorrect entry point configuration"
...
class TestCodeQuality:
    ...
    def test_class_docstrings(self):
        ...
        try:
            from src.ollamapy.ollama_client import OllamaClient
            assert OllamaClient.__doc__ is not None, "OllamaClient class missing docstring"
        except ImportError:
    ...
    def test_pytest_ini_configuration(self):
        ...
        if pytest_ini.exists():
            with open(pytest_ini, 'r') as f:
                content = f.read()
                assert '[tool:pytest]' in content or '[pytest]' in content, "pytest configuration missing"
...
class TestVersioning:
    ...
    def test_version_consistency(self):
        ...
        assert re.match(version_pattern, pyproject_version), "Version doesn't follow semantic versioning"
    ...
    def test_no_hardcoded_secrets(self):
        ...
        for py_file in src_root.rglob("*.py"):
            ...
            for pattern in secret_patterns:
                ...
                assert len(matches) == 0, f"Potential hardcoded secret found in {py_file}: {matches}"
                ...
                if dangerous in content and not content.find(f'#{dangerous}') >= 0:
                    ...
                    for line_num, line in enumerate(lines, 1):
                        if dangerous in line and not line.strip().startswith('#'):
                            pytest.fail(f"Potentially dangerous code in {py_file}:{line_num}: {line.strip()}")
