        ...
        with patch.object(sys, "argv", test_args):
            with patch.object(sys, "exit") as mock_exit:
                ...
                mock_run_vibe.assert_called_once_with(
                    model="gemma3:4b", iterations=1, analysis_model=None
                )
        ...
        with patch.object(sys, "argv", test_args):
            with patch.object(sys, "exit") as mock_exit:
                ...
                mock_run_skill.assert_called_once_with(
                    model="gemma3:4b", analysis_model="gemma3:4b", count=3, ideas=None
                )
                mock_exit.assert_called_once_with(0)
        ...
        with patch.object(sys, "argv", test_args):
            with patch.object(sys, "exit") as mock_exit:
                ...
                mock_run_editor.assert_called_once_with(
                    port=8080, skills_directory=None
                )
                mock_exit.assert_called_once_with(0)
...
class TestChatFunction:
    ...
    def test_chat_with_defaults(
        ...
        mock_analysis_engine.assert_called_once_with(
            "gemma3:4b", mock_client.return_value
        )
        mock_chat_session.assert_called_once_with(
            "gemma3:4b", mock_client.return_value, "You are a helpful assistant."
        )
        mock_ai_query.assert_called_once_with(
            mock_client.return_value, model="gemma3:4b"
        )
    ...
    def test_chat_with_custom_params(
        ...
        mock_analysis_engine.assert_called_once_with(
            "gemma2:2b", mock_client.return_value
        )
        mock_chat_session.assert_called_once_with(
            "llama3.2:3b", mock_client.return_value, "Custom system"
        )
        mock_ai_query.assert_called_once_with(
            mock_client.return_value, model="gemma2:2b"
        )
...
class TestVibeTestFunction:
    ...
    def test_run_vibe_tests_success(self, mock_vibe_tests):
        ...
        mock_vibe_tests.assert_called_once_with(
            model="test-model", iterations=5, analysis_model="gemma3:4b"
        )
    ...
    def test_run_vibe_tests_with_analysis_model(self, mock_vibe_tests):
        ...
        result = run_vibe_tests(
            model="chat-model", iterations=3, analysis_model="analysis-model"
        )
...
class TestSkillGenFunction:
    ...
    def test_run_skill_gen_defaults(self, mock_skill_gen):
        ...
        mock_skill_gen.assert_called_once_with(
            model="gemma3:4b", analysis_model=None, count=1, ideas=None
        )
    ...
    def test_run_skill_gen_custom_params(self, mock_skill_gen):
        ...
        result = run_skill_gen(
            model="custom-model", analysis_model="analysis-model", count=3, ideas=ideas
        )
